...
#define Y_MIN 40 //最も下がったとき
#define Y_MAX 70 //最も上がったとき
...
void loop(){
	unsigned char ir[7],urm[4],ir_max=0,ir_max2 =0;
	...
	if(ir[ir_max]>5){
		switch(ir_max){
			case 0:
				if(ir[0]>110){
					...
					if(ir[1]>ir[5]){
						speed =160;
						if(urm[2]>Y_MAX)cor = 300;
						else         cor = 270;
					}else{
						speed = 160;
						if(urm[2]>Y_MAX)cor = 60;
						else         cor = 90;
					}
				}
				break;
			case 1:
				if(urm[2]<Y_MIN)cor = 240;
				if(urm[2]>=Y_MIN && urm[2]<=Y_MAX)cor=270;
				if(urm[2]>Y_MAX)cor=330;
				speed = 210;
				break;
			case 2:
				if(urm[2]<Y_MIN)cor = 270;
				else cor = 330;
				speed = 210;
				break;
			case 3://真後ろ
				...
				if(urm[2]<Y_MIN)speed=0;
				else speed = 210;
				break;
			case 4:
				 if(urm[2]<Y_MIN)cor = 90;
				 else cor = 30;
				 speed = 210;
				 break ;
			case 5:
				if(urm[2]<Y_MIN)cor = 120;
				if(urm[2]>=Y_MIN && urm[2]<=Y_MAX)cor=90;
				if(urm[2]>Y_MAX)cor=30;
				speed = 210;
				break;
		}
		}else{
		...
		cor =0;
		if(urm[2]>50)speed=100;
		else speed =0;
	}
	arg = arg-GOAL;
	/*
	if(arg<5 && arg>-5 && (raw_x >1500 || raw_z >1500))arg=0;
	else {
		if(arg>0) arg=40;
		else arg=-40;
	}
